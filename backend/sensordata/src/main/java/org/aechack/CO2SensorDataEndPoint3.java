...
@ServerEndpoint(value = "/co2/{uid}", decoders = MessageDecoder.class, encoders = MessageEncoder.class)
public class CO2SensorDataEndPoint3 {
	private Session session;
	Random rand = new Random(System.currentTimeMillis());
	private static final Set<CO2SensorDataEndPoint3> clientEndpoints = new CopyOnWriteArraySet<>();
	...
	private void updateSensorValue(String sensor_uid) {
		Float val = current_values.getOrDefault(sensor_uid, 500f);
		val += rand.nextFloat() - 50f;		
	...
	public void onOpen(Session session, @PathParam("uid") String sensor_uid) throws IOException, EncodeException {
		...
		while (true) {
			...
			Float val = current_values.getOrDefault(sensor_uid, 700f);
			current_values.put(sensor_uid, val);
